...
import { ApiResponse, Chart, ChartFilterOptions, ChartSummary } from '../types/chart.types';
...
export function createChartsRouter(chartService: ChartService): Router {
  ...
  router.get('/', async (req: Request, res: Response) => {
    ...
    const filters: ChartFilterOptions = {};

    const dl = parseIntParam(detailLevel);
    if (dl !== undefined) filters.detailLevel = dl;

    const ps = parseIntParam(processStep);
    if (ps !== undefined) filters.processStep = ps;

    const seg = parseIntParam(segment);
    if (seg !== undefined) filters.segment = seg;
    ...
    if (typeof chartType === 'string' && chartType.trim()) {
      filters.chartType = chartType.trim() as ChartFilterOptions['chartType'];
    }
    ...
    const response: ApiResponse<ChartSummary[]> = {
      success: true,
      platform: platformId,
      data: summaries,
      meta: {
   ...
   *               $ref: '#/components/schemas/ErrorResponse'
   */
  router.get('/:chartId', async (req: Request, res: Response) => {
    ...
    const response: ApiResponse<Chart> = {
      success: true,
      platform: platformId,
      data: chart,
    };
   ...
   *               $ref: '#/components/schemas/ErrorResponse'
   */
  router.get('/:chartId/children', async (req: Request, res: Response) => {
    const { platformId, chartId } = req.params;
